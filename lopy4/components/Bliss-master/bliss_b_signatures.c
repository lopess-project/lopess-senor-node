...
void bliss_signature_retrieve_params(bliss_signature_t *signature, char *sig, const bliss_public_key_t *public_key) {
    uint32_t n, kappa, i;
    int32_t *z1, *z2;
    uint32_t *c_indices;
    bliss_param_t p;
    ...
    int sig_count = 0;
    for(i=0; i<23; i++) {
        uint32_t idx = (uint32_t) c_indices[i];
        char intValue[4];
        sprintf(intValue, "%d", idx);
        for(int8_t j=0; j<4; j++) {
            sig[sig_count] = intValue[j];
            sig_count++;
        }    
    }
    sig[sig_count] = '\n';
