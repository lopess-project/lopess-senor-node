...
void hss_hash_ctx(void *result, int hash_type, union hash_context *ctx,
    ...
    case HASH_SHA256: {
        ...
        mbedtls_sha256_update(&ctx->sha256, message, message_len);
        mbedtls_sha256_finish(&ctx->sha256, result);
#if ALLOW_VERBOSE
        if (hss_verbose) {
            printf( " ->" );
...
void hss_update_hash_context(int h, union hash_context *ctx,
    ...
    switch (h) {
    case HASH_SHA256:
        mbedtls_sha256_update(&ctx->sha256, msg, len_msg);
        break;
    }
}
...
void hss_finalize_hash_context(int h, union hash_context *ctx, void *buffer) {
    switch (h) {
    case HASH_SHA256:
        mbedtls_sha256_finish(&ctx->sha256,buffer);
#if ALLOW_VERBOSE
    if (hss_verbose) {
        printf( " -->" );
